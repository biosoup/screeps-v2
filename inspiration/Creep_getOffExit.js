...
Creep.prototype.getOffExit = function() {
    ...
    if (directionsFromExit['x'][this.pos.x]) {		// Are we on the left / right exits?
        var allowedDirections = directionsFromExit.x[this.pos.x]
    }
    else if (directionsFromExit['y'][this.pos.y]) {	// or are we on the top / bottom exits?
        var allowedDirections = directionsFromExit.y[this.pos.y]
    }
    
    if (!allowedDirections) { 	// Not on an exit tile
