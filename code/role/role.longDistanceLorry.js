            ...
            if (creep.room.name == creep.memory.home && (creep.carry.energy == 0 || _.isEmpty(creep.memory.target))) {
                ...
                if (_.size(allContainers) > 0) {
                    ...
                    if (!_.isEmpty(validTarget)) {
						...
						let container = Game.getObjectById(validTarget.id)
                        ...
                        if (false) {
                            console.log(creep.name + " going for " + validTarget.id + " " + r.memory.containerSources[validTarget.id].energy + " in distance " + validTarget.distance + " for a return of e/d " + validTarget.ed.toFixed(2) + " (CPU used: " + (Game.cpu.getUsed() - cpuStart).toFixed(2) + ")")
                        }
                    } else {
                        console.log(creep.name + " ERRRRR!!!  target not valid " + JSON.stringify(validTarget) + " " + JSON.stringify(allContainers))
