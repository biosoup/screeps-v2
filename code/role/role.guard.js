    ...
    nonTask: function(creep) {
        ...
        if (creep.room.name == creep.memory.target) {
            ...
            if (creep.pos.y == 49) {
                ...
                var step = new RoomPosition(48, creep.pos.y+_.random(1), creep.room.name)
                creep.travelTo(step)
            } else if (creep.pos.y == 0) {
            ...
            if (!_.isEmpty(hostile)) {
                ...
                if ((Game.time % 3) == 0) {
                    ...
                    if (creep.room.name == creep.memory.home) {
                        //search for white flags to go defend
                        var whiteFlags = _.first(_.filter(Game.flags, (f) => f.color == COLOR_WHITE && _.last(_.words(f.name, /[^-]+/g)) == creep.room.name))
                        if (!_.isEmpty(whiteFlags)) {
                            creep.say(EM_FLAG, true)
                            console.log("ENEMY IN " + whiteFlags.room.name + " dispatching GUARD")
                            if (!_.isEmpty(whiteFlags.room.name)) {
                                creep.memory.target = whiteFlags.room.name;
