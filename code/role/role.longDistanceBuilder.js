    ...
    newTask: function(creep) {
        ...
        if (!_.isEmpty(creep.memory.target) && creep.room.name != creep.memory.target) {
            ...
            if (creep.carry.energy > 0) {
                ...
                //check forw GREY claim flags
                var greyFlag = creep.room.find(FIND_FLAGS, {
                    filter: (f) => f.color == COLOR_GREY && f.room == creep.room
                })
                if (greyFlag.length > 0) {
                    if (creep.room.controller.my) {
                        var existingCs = greyFlag[0].pos.lookFor(LOOK_CONSTRUCTION_SITES)
                        if (existingCs.length == 0) {
                            creep.room.createConstructionSite(greyFlag[0].pos, STRUCTURE_SPAWN)
                        } else {
                            //preemptive safemode
                            if(creep.room.controller.safeModeAvailable > 0 && creep.room.controller.level < 3) {
