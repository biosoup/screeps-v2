...
class Traveler {
		...
		var callback = (roomName) => {
			...
			if (room) {
				...
				if (options.obstacles) {
					matrix = matrix.clone();
					for (var obstacle of options.obstacles) {
	...
	static findRoute(origin, destination, options = {}) {
		...
		var allowedRooms = { [origin]: true, [destination]: true };
		var highwayBias = 1;
	...
	static addStructuresToMatrix(room, matrix, roadCost) {
		...
		for (var structure of room.find(FIND_STRUCTURES)) {
		...
		for (var site of room.find(FIND_MY_CONSTRUCTION_SITES)) {
		...
		for (var structure of impassibleStructures) {
